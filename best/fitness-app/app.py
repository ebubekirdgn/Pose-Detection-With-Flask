...
class RegisterForm(FlaskForm):
    ...
    username = StringField(
        "Kullanıcı Adı", validators=[DataRequired(), Length(min=4, max=25)]
    )
    ...
    date_of_birth = StringField(
        "Doğum Tarihi (YYYY-MM-DD)", validators=[DataRequired()]
    )
    ...
    if form.validate_on_submit():
        ...
        if existing_user:
            ...
            if form.password.data != form.confirm_password.data:
                ...
                try:
                    conn.execute(
                        "INSERT INTO users (first_name, last_name, username, password, date_of_birth, height, weight) VALUES (?, ?, ?, ?, ?, ?, ?)",
                        (
                            first_name,
                            last_name,
                            username,
...
#         "layout.html", user=user, totals=totals
#     )  # Toplamları sayfaya gönder
...
def layout():
    ...
    # Tüm stratejileri içeren bir yapı
    strategies = {
    ...
    # Her bir strateji için toplam değerleri al
    totals = {
        'total_biceps_curl': 0,
        'total_triceps_extension': 0,
        'total_lateral_raise': 0,
        'total_squat': 0,
        'total_shoulder_press': 0,
        'total_crunch': 0,
    }

    for exercise_name, strategy in strategies.items():
        exercise_totals = strategy.get_totals(user)  # Her stratejinin toplamını al
        totals.update(exercise_totals)  # Totals sözlüğüne ekle
...
def biceps_curl():
    ...
    return render_template(
        "components/biceps_curl.html", user=session["user"], totals=totals
    )
...
def triceps_extension():
    ...
    user = session["user"]
    triceps_extension_strategy = (
        TricepsExtensionStrategy()
    )  # CrunchStrategy sınıfınızı oluşturmalısınız
    totals = triceps_extension_strategy.get_totals(user)  # Toplam değerleri al
    return render_template(
        "components/triceps_extension.html", user=user, totals=totals
    )
...
def lateral_raise():
    ...
    user = session["user"]
    lateral_raise_strategy = (
        LateralRaiseStrategy()
    )  # CrunchStrategy sınıfınızı oluşturmalısınız
...
def crunch():
    ...
    return render_template(
        "components/crunch.html", user=user, totals=totals
    )  # Toplamları sayfaya gönder
...
def finish_stream(exercise_name):
    ...
    counter_value = (
        strategy.get_counter()
    )  # Seçilen strateji üzerinden sayaç değerini al
    ...
    if counter_value > 0:
        ...
        existing_record = conn.execute(
            "SELECT * FROM exercises WHERE user = ? AND created_date = ?",
            (user, current_date),
        ).fetchone()
        ...
        if existing_record:
            ...
            new_count = (
                existing_record[exercise_name] + counter_value
            )  # Dinamik güncelleme
            conn.execute(
                f"""
                UPDATE exercises
